...
KAPI void* kallocate(u64 size, memory_tag tag){
    if (tag == MEMORY_TAG_UNKONW) {
        KWARN("kallocate called using MEMORY_TAG_UNKNOWS. Re-class this allocation.");
    }
...
KAPI void kfree(void* block, u64 size, memory_tag tag) {
    if (tag == MEMORY_TAG_UNKONW) {
        KWARN("kfree called using MEMORY_TAG_UNKNOWS. Re-class this allocation.");
    }
